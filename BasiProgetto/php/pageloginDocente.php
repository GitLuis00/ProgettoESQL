   ...
   if (!empty($_POST["email"]) and !empty($_POST["password"])) {
      ...
      try {
        ...
        if (strlen($email) <= 30 && strlen($password) <= 30) {
            $res = $pdo->prepare('CALL VerificaCredenzialiDocente(?,?)');
            ...
            if ($result['counter'] == 1) { // Modifica 'authorized' in base al nome della colonna ritornata dalla tua SP
                ...
                $stmt = $pdo->prepare("SELECT Docente.email 
                       FROM Docente 
                       JOIN Utente ON Docente.email = Utente.email
...
if ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    ...
    $_SESSION['email_docente'] =  $emailDocente;
    // Qui puoi fare ulteriori operazioni con l'ID dello studente
    
}// Salva l'ID del docente nella sessione
                ...
                header("Location: pageloginDocente.php?error=invalid_credentials");
                exit();
            }
        } else {
            // Casi in cui email o password superano la lunghezza massima
            header("Location: pageloginDocente.php?error=input_too_long");
            exit();
        }
...
<html>
<head>
    <title>ESQL - Login Docente</title>
...
<form action='pageloginDocente.php' method="post">
    ...
    <h2 class="titolo">LOGIN DOCENTE</h2>
